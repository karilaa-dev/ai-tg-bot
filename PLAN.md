...
The Alpine runner includes pinned OfficeCLI PPTX/DOCX skills and the OfficeCLI binary, but no browser. The model creates and validates editable Office files in OpenSandbox and exports self-contained page HTML through the protected bot outbox. The bot-only `render_office_preview` tool sends that HTML to an optional external Browserless Playwright or REST endpoint, returns the screenshot only to model context for visual QA, and removes screenshot base64 from persisted tool details. Browserless settings and credentials never enter OpenSandbox.
...
- OfficeCLI routing, scoped HTML export, Browserless URL/auth safety, remote rendering cleanup, image validation, and base64-free persisted preview details.
