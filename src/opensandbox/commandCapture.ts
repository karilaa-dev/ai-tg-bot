...
const OUTPUT_SENTINEL_BYTES = 1;
const TRUNCATION_MARGIN_BYTES = 4;

export function commandOutputReadLimit(maxOutputChars: number): number {
  return Math.min(
    Number.MAX_SAFE_INTEGER,
    maxOutputChars * UTF8_MAX_BYTES_PER_CHAR + TRUNCATION_MARGIN_BYTES + OUTPUT_SENTINEL_BYTES,
  );
}
...
export function buildBoundedCommandCapture(input: {
  ...
  const captureLimit = commandOutputReadLimit(input.maxOutputChars) - OUTPUT_SENTINEL_BYTES;
  ...
  const script = [
    ...
    "stderr_capture_pid=$!",
    `${shellJoin([input.command, ...input.args])} < ${quoteShellToken(input.stdinPath)} >&3 2>&4`,
    "command_status=$?",
    "exec 3>&- 4>&-",
