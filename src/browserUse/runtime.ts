...
export class BrowserUseRuntimeManager {
  ...
  private async snapshot(
    ...
    return this.withTab(userId, threadId, tabId, signal, async (session, tab) => {
      const { refs, output } = await inspectPage(tab.page, offset, includeScreenshot);
      tab.refs = refs;
      return { ...output, tab_id: tab.id, session_remaining_seconds: remainingSeconds(session) };
    });
  }
  ...
  private async screenshot(
    ...
    return this.withTab(userId, threadId, tabId, signal, async (session, tab) => {
      ...
      return { ...await capturePage(tab.page, fullPage), session_remaining_seconds: remainingSeconds(session) };
    });
  }
...
async function browserStorageSignature(context: BrowserContext): Promise<string | null> {
  ...
  return state ? sha256Hex(JSON.stringify(state)) : null;
}
