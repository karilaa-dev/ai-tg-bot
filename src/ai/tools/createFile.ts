...
import { assertCreatedFileCapacity, normalizeCreatedFilePath, prepareCreatedFile, toToolError } from "./helpers.js";
...
export function createCreateFileTool(input: ToolBuildInput) {
  return defineBotTool({
    ...
    execute: async ({ path: virtualPath, name, mime, caption, delivery = "auto" }, signal) => {
      try {
        const sourceVirtualPath = normalizeCreatedFilePath(virtualPath);
        ...
        if (replaced) input.outgoingBuffers?.release(replaced);
