...
export function createCreateFileTool(input: ToolBuildInput) {
  return defineBotTool({
    description:
      `Queue a file that you created in this thread's bash workspace to send back to the Telegram user. First create the file with bash, then call this tool with its absolute virtual path. Attach at most ${MAX_CREATED_FILES_PER_ANSWER} files per answer; do not call create_file more than ${MAX_CREATED_FILES_PER_ANSWER} times in one answer. If more files are needed, send the first ${MAX_CREATED_FILES_PER_ANSWER} and say the rest can be sent in another answer. Files up to ${MAX_FILE_MB} MB are allowed unless they are native/compiled executables such as exe, dll, ELF/Mach-O binaries, shared libraries, Java bytecode archives, or WebAssembly. Scripts and source files such as sh, bash, ps1, py, js, ts, and similar text/code files are allowed. Images are sent as Telegram photos by default; set delivery to document when exact bytes, transparency, metadata, print/source assets, or uncompressed delivery matters.`,
