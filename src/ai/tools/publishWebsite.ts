...
export function createPublishWebsiteTool(input: ToolBuildInput) {
  return defineBotTool({
    ...
    description:
      "Publish an HTTP server already running persistently in this thread's E2B sandbox and bound to 0.0.0.0. Put the site in a dedicated workspace subdirectory, start the server from that exact directory with stdin and output detached (for example: nohup command </dev/null >server.log 2>&1 &), then pass that directory as site_dir. The harness rejects workspace-root and Telegram-file serving and checks the listener process directory. The returned HTTPS URL is public and unauthenticated, so do not publish secrets. The result reports when the sandbox will pause; publishing again explicitly resumes it.",
