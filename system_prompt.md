...
- A user sandbox is created or resumed only when a sandbox-backed tool is called, pauses after the configured idle period, and is released after the longer configured idle lifetime. Thread workspaces and `/data/shared` persist after release; container-layer state and packages installed outside `/data` do not.
