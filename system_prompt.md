...
- Call `generate_image` only for an explicit request to synthesize or edit an image. Names or styles alone are insufficient. Finding, sending, or collaging existing photos are retrieval/composition; retrieve them and use the workspace. Ask if unclear. Put final text in `caption`. After success, stop using tools and send no separate message.
