...
The bot derives its default private template from the application version. Version `2.0.1` uses `ai-tg-bot-tools:v2.0.1`. The template in [`e2b-template`](e2b-template/README.md) uses E2B Base with 2 vCPU and 2 GiB RAM. It includes OfficeCLI, the OpenSCAD `2026.08.23` Node/WebAssembly engine with POV-Ray `3.7.0.10`, `openscad-build`, ImageMagick, archive tools, Python, Node.js, Git and SSH clients, SQLite, compilers, and standard shell diagnostics. It does not install an X server, OpenGL renderer, Chromium, or browser automation packages.
...
```dotenv
E2B_API_KEY=<secret>
# Optional override. The default for version 2.0.1 is ai-tg-bot-tools:v2.0.1.
# E2B_TEMPLATE=ai-tg-bot-tools:v2.0.1
