...
file-reused = ♻️ Reused indexed file <code>{ $name }</code>.
docling-unavailable = ⚠️ This document needs Docling conversion, but Docling is disabled or unavailable. Configure DOCLING_URL with an external Docling server and try again.
docling-conversion-failed = ⚠️ Docling could not convert this document. Check that the file is valid and supported, then try again.
